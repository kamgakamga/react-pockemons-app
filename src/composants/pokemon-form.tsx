...
type Field = {
  ...
  error?: string,//message d'erreur si valeur n'est pas valide.
  isValid?: boolean  //boolean true= champs valide false = champs non valide
}
...
const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) =>{
   ...
   setForm({...form, ...newField});// spread operator permet de fusionné deux objets
  }
  ...
  const selectType = (type: string,e: React.ChangeEvent<HTMLInputElement>): void =>{
   ...
   if(checked){
    ...
    // si l'utilisateur décoche un type, on retire à la liste des types du pokémon
...
if(!form.picture.value.startsWith(start) || !form.picture.value.endsWith(end) ){
  ...
  const errorMsg: string  = '';
   const newField:Field = { value: form.picture.value,error: errorMsg,isValid: true};
   newForm = {...newForm, ...{picture:newField}};
}
}
    ...
    if(!/^[a-zA-Zàéè ]{3,25}$/.test(form.name.value)){
      ...
      const errorMsg: string  = '';
       const newField:Field = { value: form.name.value,error: errorMsg,isValid: true};
       newForm = {...newForm, ...{name:newField}};
    }
    //validator hp
    if(!/^[0-9]{1,3}$/.test(form.hp.value)){
      ...
      const errorMsg: string  = '';
       const newField:Field = { value: form.hp.value,error: errorMsg,isValid: true};
       newForm = {...newForm, ...{name:newField}};
    }
    //validator cp
      ...
      const errorMsg: string  = '';
       const newField:Field = { value: form.cp.value,error: errorMsg,isValid: true};
       newForm = {...newForm, ...{name:newField}};
    }
                ...
                <div className="form-group">
                  ...
                  {types.map(type => (
                    <div key={type} style={{marginBottom: '10px'}}>
                      <label>
                        <input id={type} disabled={!isTypesValid(type)} type="checkbox" name="types" className="filled-in" value={type} checked={hasType(type)} onChange={e => selectType(type,e)}></input>
                        <span>
